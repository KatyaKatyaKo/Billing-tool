...
public class GestionTableDesCommandes41 {
	private ES ES = new ES();
	
	private GestionUneCommande41 gestCde = new GestionUneCommande41(); //Pour gérer une Commande
	...
	//************************************** MENU GESTION DES COMMANDES ****************************************
			...
			public void menuGeneral(TableDesCommandes41 tabCde, TableArticle41 tabArt) throws AbandonException { 
				int choix;
				do {
	...
	//------------------------------------METODES GESTION DES COMMANDES --------------------------------
			...
			public void creer(TableDesCommandes41 tabCde, TableArticle41 tabArt, int numCde) throws AbandonException { //A REVOIR
				//On cree une commande
				UneCommande41<Integer> cde = new UneCommande41<Integer>();
			...
			//Boucle infinie
			public static int premierNumDispo(TableDesCommandes41 tabCde) { //CHAQUE FOIS LE COMPTEUR COMMENCE PAR 1 ==>
				int compteur = 1;
				do {
			...
			public void supprimer(TableDesCommandes41 tabCde)  throws AbandonException { //ON TRAITE LES EXCEPTIONS PAS ICI MAIS DANS LE CLIENT jAVA ==> THROWS
			...
			//MODIFIER une COMMANDE....3
					...
					/**
					private void modifier(TableDesCommandes41 tabCde, TableArticle22 tabArt) {
						if(tabCde.taille()==0) {
							ES.affiche("\n\t LA TABLE des COMMANDES est VIDE");
						}
						else {
					...
					*/
					...
					private void modifier(TableDesCommandes41 tabCde, TableArticle41 tabArt) throws AbandonException {
						int numCde = (int)ES.saisie("\n\t SAISISSEZ le NUMERO de COMMANDE a MODIFIER:\n\t "+ tabCde.cle() + ":", 1, Integer.MAX_VALUE);
						UneCommande41 <Integer> cde = tabCde.retourner(numCde);
						if(cde==null) {/*Afficher mssage*/ }
						//else {gestCde.menuGeneral(tabCde, tabArt, numCde);}
						else {gestCde.menuGeneral(cde, tabArt, numCde);}
						
						//(TableDesCommandes41 tabCde, TableArticle22 tabArt, int numCde)
						
					}
			
			//AFFICHER une COMMANDE....4
			public void visualiser(TableDesCommandes41 tabCde) throws AbandonException {
				if(tabCde.taille()==0) {
			...
			public void visualiserToutesCommandes(TableDesCommandes41 tabCde) { //case 4: visualiserToutesCommandes
				if(tabCde.taille()==0){
			...
			private void facturer(TableDesCommandes41 tabCde, TableArticle41 tabArt) {
			if(tabCde.taille()==0){
				ES.affiche("\n\t IL N Y A PAS DE COMMMANDES A FACTURER");
			}
			else {
				//for (UneCommande41 Commande: tabCde)
