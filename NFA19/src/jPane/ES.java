...
public class ES implements interfaceIO {
	//static ou pas??
	public static void affiche(String message) {JOptionPane.showMessageDialog(null, message);} //pointe sur elle même

	public String saisie(String mes) {
		return mes = JOptionPane.showInputDialog(mes);
	}
	...
	public Object saisie(String message, Object ...param) throws AbandonException{
		...
		//JPane ne permet pas de lire un init, seulement les String -->
		do {
		try {
		 data = JOptionPane.showInputDialog(null, message,"Superette", JOptionPane.PLAIN_MESSAGE); //affiche un
	...
	public boolean saisieOuiNon(String mes) {
		return JOptionPane.showConfirmDialog(null, mes)==0; //Oui = 0
	}
}
...
/*
	public int saisie(String message, Object ...param) throws AbandonException{
		int mini = (Integer)param[0];
		int max;
		if(param.length==2) {
			max = (Integer) param[1];
		}
		...
		try {
		 ...
		 //data = JOptionPane.showInputDialog(/*null,*//* message/*,"MESSAGE (title)"*/ //,YES_NO_OPTION==0*/); //affiche un
		 ...
		 //verifier la conditon d'intervalle
		 if(valeur>=mini&&valeur<=max)break;
		 affiche("Saisie hors intervalle de "+ mini+ "à " + max + "resaisissez svp") ;
		 }
		 catch (NumberFormatException nfe){
			 if(data.equals("")) affiche("Il faut saisir qqch");
			 affiche("Saisie non numérique. resaisissez)");}
			...
			if (saisieOuiNon("Voulez vous abandonner"))
			throw ae; // c 'est cette exception qui permet d'abandonner l'application. Elle est géré dans le main
		}
		}
		while(true);
		 return valeur;
